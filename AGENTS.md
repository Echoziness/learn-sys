...
- 审核轮次硬上限 2（`MAX_REVIEW_RETRIES`，打回重写最多 1 次），超限放行、交付仅保留 supported 论断。
...
- **Web 端点补充**（W2）：`GET /api/sessions`（列表，回放入口）；`GET /{id}/stream?after_seq=`（实时 SSE 订阅：先 subscribe 再补读历史、按 seq 去重，裁判面跟随另一 tab）；`GET /{id}/replay?format=json`（带 seq 的 JSON 数组，播放器步进用）；`POST /{id}/end`（session_end 收口）；`GET /{id}/topics/{entry}/state`（needs_teaching/scaffold_pending/followup_last/prereq_id——Web 工作台镜像 CLI 状态机与刷新恢复的依据）；`POST /{id}/topics/{entry}/followup`（动态追问：判定+记录困惑+直接解答，不即时出题，2026-08-28 设计回归）；`GET /{id}/exports`（条目化导出产物，知识库同构条目，未导出时空数组）；`POST /{id}/export`（主动触发条目化导出，含误区提炼）+ `GET /{id}/export/download`（同构 jsonl 下载，已删会话的孤儿产物亦可下载）；`DELETE /{id}?keep_packages&keep_exports`（删会话与过程数据，产物可额外保留为孤儿行）；`GET /api/resources`（跨会话聚合资源库，`?session_id&entry_id` 筛选，/resources 页面数据源）；
...
- **动态追问（记录困惑→解答→下轮注入，2026-08-28 设计回归）**：学生提问 → 判定有效性 → 有效则**落困惑记录并直接给出解答**（不即时反问确认题——学生正因困惑提问，需要的是答案）；困惑轮 = topic_rounds 侧车轮（question_type='followup'、decision='followup'，answer 字段承载系统解答），不写掌握度，`main_rounds` 过滤后主状态机不受影响；**消化从历史推导**：困惑轮号 > 最近 teach_delivered 轮号 = 未消化 → 下一轮教学注入 `followup_context`（错因回流同通道，generate 针对性讲解），教学完成后自然消化；有未消化困惑时 needs_teaching 强制 true（巩固模式让位）；困惑记录进 distill 原料（与错题同管道，学生疑问=困惑信号，系统解答不采集）；导出管线在 `core/export_pipeline.py`（脚本为薄壳，Web 触发端点复用同一管线）；旧式确认题事件（followup_offered/graded）仅历史回放兼容，不再产生；
